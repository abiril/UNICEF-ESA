...
And define the distance matrix as the distances between all points for each country
...
for (i in length(countries)) {
    ...
    xy[[i]] <- country.data[[i]][, c("x", "y")]
    distance.matrix[[i]] <- as.matrix(dist(xy[[i]]), ncol = nrow(xy[[i]]))
}
...
for (i in 1:length(countries)) {
    ...
    data.one <- country.data[[i]][, predictor.variable.names]

    vars <- spatialRF::auto_cor(
        x = data.one,
        cor.threshold = 0.6,
    ...
    #pred.vars[[i]] <- vars$selected.variables
}

vars <- spatialRF::auto_cor(
        x = country.data[[i]][, predictor.variable.names],
        cor.threshold = 0.6,
        preference.order = preference.order 
    ) %>% 
    spatialRF::auto_vif(
...
pred.vars[[1]]$selected.variables
pred.vars[[2]]$selected.variables
pred.vars[[3]]$selected.variables
...
n_features <- 14 ####NOOOOOO THIS IS LESS AFTER MULTICOL
...
rmse <- list()
for (i in 1:length(countries)) {
    ind[[i]] <- sample(2, nrow(country.data[[i]]), replace = TRUE, prob = c(tuned_hyper[[i]][1,4], 1 - tuned_hyper[[i]][1,4]))
    ...
    saveRDS(rf2[[i]], file = paste("Outputs/", countries[[i]], "_10km_school_count_RF_Tuned.rds", sep = ""))
...
for (i in 1:length(countries)){
    ...
    CM[[i]][[3]] <- confusionMatrix(predicted[[i]]$p1, country.data[[i]]$SCHOOLS)$table
    S[[i]][[7]] <- confusionMatrix(predicted[[i]]$p1, country.data[[i]]$SCHOOLS)$overall[1]
    S[[i]][[8]] <- confusionMatrix(predicted[[i]]$p1, country.data[[i]]$SCHOOLS)$byClass[1]
    S[[i]][[9]] <- confusionMatrix(predicted[[i]]$p1, country.data[[i]]$SCHOOLS)$byClass[2]
...
prob.pred.r <- list()
for (i in 1:length(countries)){
    p4[[i]] <- predict(rf2[[i]], country.data[[i]])
